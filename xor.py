...
from phenomes import FeedForwardCPPN
...
def xor(genomes):
    ...
    for genome_key, genome in genomes:
        cppn = FeedForwardCPPN.create(genome)
...
# Run population on the defined task for the specified number of generations and collect the winner.
winner_genome = pop.run(xor, goal_fitness, num_generations)

# Decode winner genome into CPPN representation
cppn = FeedForwardCPPN.create(winner_genome)

# Decode Substrate from CPPN
substrate = decode(cppn, sub_in_dims, sub_o_dims, sub_sh_dims)

# Visualize networks of CPPN and Substrate. Files are saved in reports/champion_images
